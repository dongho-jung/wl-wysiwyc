...
and `Enter` click the target, `=` right-clicks it, and either click key
with `Shift` double-clicks it; `keys.left_click` takes a list, so give
it as many keys as you like. A hint's keys are where its target is: the
keyboard is laid over the window, so on qwerty something in the
bottom-left corner is labelled `z` and something in the top-right `p`.
Set `keys.layout` for dvorak, or for no layout at all, and
`keys.excluded` to keep letters you would rather not reach for out of
the labels. `Space` switches between hints and the letter grid unless
you have given it another job, `Tab` picks another window with `1`-`9`,
