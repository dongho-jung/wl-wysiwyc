...
letter grid unless you have given it another job, `Tab` picks another
window with `1`-`9`, `Backspace` undoes one press, and `Esc` backs out
and then quits. Set `keys.reset` to the key that opens the overlay:
pressing it starts the choices over, and pressing it with nothing to
undo closes the overlay. `Ctrl+Esc` gets the keyboard back if a run is
